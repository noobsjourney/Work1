...
\begin{figure}[t]
  ...
  trajectory, and transmission power to ensure reliable communication.}
...
\label{eq:SU_Motion}
\end{equation}
where $\phi_{s,k}[n] \in [-\pi,\pi]$ represents the direction of the $k$-th SU at time slot $n$.
...
\end{align}
\end{subequations}
where $P_B$ and $P_J$ denote the transmission power of the PBS and the J-UAV, respectively.
...
and transmission power adaptation problem in a dynamic spectrum sharing UAV network 
...
\\
\text{s.t.}\quad
& \sum_{m=1}^{M} R^{\mathrm{p}}_{m,j,n} \ge R^{\min}_{j},
...
$\mathbf{P}=\{P_C[n]\}_{n\in\mathcal N}$ represents the transmission power adaptation 
