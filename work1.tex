...
As shown in Fig.~\ref{fig:exp1}, the convergence of the proposed scheme is presented with respect to the training episodes. All three schemes exhibit an increasing trend and eventually converge as training proceeds. 
Significant reward fluctuations appear in the early stage due to random exploration and jammer mode switching, while the curves become stable after around 400 episodes. 
The proposed scheme converges faster and achieves the highest reward, because the inferred jammer-mode information enhances the state representation under non-stationary interference.
The Hybrid PPO-DDPG baseline performs worse than MA2C-DDPG, indicating that algorithmic complexity alone cannot guarantee performance gains without effective mode-aware state augmentation.

Fig.~\ref{fig:exp2} shows the recovery comparisons of the proposed scheme with baseline schemes.

Fig.~\ref{fig:expx3} shows the overall performance comparisons of the proposed scheme with baseline schemes.
The proposed scheme achieves the highest average SU sum rate and the lowest outage probability.
This is because the proposed framework can better optimize spectrum allocation, UAV control, and transmit power under dynamic interference. 
Moreover, the gain over Hybrid PPO-DDPG demonstrates the importance of jammer-mode information. 
In addition, MA2C-DDPG is inferior to the proposed scheme because it does not exploit mode-aware state augmentation.
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{exp/exp1_convergence.png}
  \caption{Convergence comparisons of our proposed scheme with baseline schemes.}
  \label{fig:exp1}
\end{figure}
\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{exp/exp2_recovery.png}
  \caption{Recovery comparisons of our proposed scheme with baseline schemes.}
  \label{fig:exp2}
\end{figure}
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{exp/exp3_overall.png}
  \caption{Overall comparisons of our proposed scheme with baseline schemes.}
  \label{fig:expx3}
\end{figure}
