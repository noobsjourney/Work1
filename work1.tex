...
In this paper, in order to address these issues, a mode-aware anti-jamming framework for DSS-enabled UAV networks is proposed. 
A CNN–GRU based module is first developed to recognize jamming modes from short-term interference observations. 
The inferred mode information is then incorporated into a semantic-augmented Markov decision process. 
Based on this formulation, a hybrid reinforcement learning framework combining proximal policy optimization (PPO) and 
deep deterministic policy gradient (DDPG) is designed to jointly optimize spectrum allocation, UAV trajectory, and transmit power.
Simulation results demonstrate that the proposed approach achieves faster adaptation to jamming mode transitions and 
significantly improves communication reliability and spectrum efficiency compared with benchmark schemes.
