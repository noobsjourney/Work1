...
At each time slot, the C-UAV obtains the jamming mode belief $\boldsymbol{\pi}[n]$, constructs the
semantic-augmented state $\mathbf{s}[n]$, and jointly determines the spectrum allocation $\mathbf{a}_d[n]$
and the continuous control action $\mathbf{a}_c[n]$ via PPO and TD3, respectively.
PPO is trained on-policy, while TD3 is updated off-policy with an experience replay buffer,
enabling stable learning under pattern-switching jamming.
