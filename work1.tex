...
\end{equation}
where $\mathbf{a}_d[n]$ is the discrete subcarrier allocation decision (equivalently,
$\{\rho_{k,n}[m]\}$), and $\mathbf{a}_c[n]$ is the continuous control vector given by
\begin{equation}
\mathbf{a}_c[n] = \big(\phi_c[n],\, P_C[n]\big),
\end{equation}
with $\phi_c[n]\in[-\pi,\pi]$ and $P_C[n]\in[P_C^{\min},\,P_C^{\max}]$.
...
\begin{equation}
r[n]=\sum_{k=1}^{K} R_k[n]-\lambda_{\mathrm{out}}\sum_{k=1}^{K}\mathbb{I}^{\mathrm{out}}_{k,n}-\lambda_{\mathrm{pu}}\sum_{j=1}^{J}\delta_j[n],
\label{eq:reward_new}
\end{equation}
where $\mathbb{I}^{\mathrm{out}}_{k,n}=\mathbb{I}\{R_k[n]<R_{\mathrm{th}}\}$ and
$\lambda_{\mathrm{pu}}>0$ controls the penalty strength for PU protection.
Since the outage penalty is applied on a per-slot basis,
prolonged service interruption
results in a larger cumulative loss, thereby encouraging fast recovery after jamming mode transitions.
...
parameterized by $\theta$, where $\mathbf{a}_d[n]$ denotes the subcarrier allocation decision
