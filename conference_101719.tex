...
=
- \sum_{c=1}^{|\mathcal{Z}|} y_c[n]\log\big(\pi_c[n]\big),
\end{equation}
and is used online only for inference to output $\boldsymbol{\pi}[n]$.
\subsection{Semantic-Aware MDP Formulation}
Following the MDP formulation in the reference work, we incorporate the mode probability vector
...
\begin{equation}
\mathbf{s}[n] = \big( \mathbf{q}[n],\, \mathbf{g}[n],\, \boldsymbol{\pi}[n] \big),
\end{equation}
where $\mathbf{q}[n]$ denotes the UAV position, $\mathbf{g}[n]$ represents channel and
...
Let $\pi_\psi(\mathbf{a}_s|\mathbf{s})$ denote the stochastic policy for spectrum allocation
parameterized by $\psi$, where $\mathbf{a}_s[n]$ represents the subcarrier allocation decision
...
\begin{equation}
\mathcal{L}^{\mathrm{PPO}}(\psi)=\mathbb{E}_{n}\Big[\min\!\big(r_n(\psi)\hat{A}_n,\,\mathrm{clip}(r_n(\psi),1-\epsilon,1+\epsilon)\hat{A}_n\big)\Big],\tag{20}
\end{equation}
where
\begin{equation}
r_n(\psi)=\frac{\pi_\psi(\mathbf{a}_s[n]|\mathbf{s}[n])}{\pi_{\psi_{\mathrm{old}}}(\mathbf{a}_s[n]|\mathbf{s}[n])},\tag{21}
\end{equation}
...
\begin{equation}
\hat{A}_n=\sum_{l=0}^{\infty} (\gamma\lambda)^l \delta_{n+l},\quad\delta_n=r[n] + \gamma V(\mathbf{s}[n+1]) - V(\mathbf{s}[n]),\tag{22}
\end{equation}
...
Let $\mu_\varphi(\mathbf{s})$ output the continuous control $a_t[n]$.
TD3 employs two critics $Q_{\omega_1}$ and $Q_{\omega_2}$, and constructs the target
\begin{equation}
y[n]=r[n]+\gamma\min_{i=1,2}Q_{\omega_i^-}\big(\mathbf{s}[n+1],\,\tilde{a}_t[n+1]\big),
\end{equation}
where
\begin{equation}
\tilde{a}_t[n+1]=\mu_{\varphi^-}(\mathbf{s}[n+1]) + \eta
\end{equation}
with clipped smoothing noise $\eta$.
The critics are trained by minimizing the squared TD error, while the actor is updated
in a delayed manner by maximizing $Q_{\omega_1}(\mathbf{s},\mu_\varphi(\mathbf{s}))$.

At each slot, the C-UAV obtains $\boldsymbol{\pi}[n]$ from the recognizer, constructs
the semantic-augmented state $\mathbf{s}[n]$, and outputs $\mathbf{a}_s[n]$ via PPO and
$a_t[n]=\phi[n]$ via TD3. PPO is updated using on-policy trajectories, while TD3 is
